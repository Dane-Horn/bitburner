...
export function hackPercent(server: Server, player: Player){
...
export function growPercent(server: Server, threads: number, player: Player, cores = 1) {
...
export function getWeakenTime(server: Server, player: Player) {
 
    const weakenTimeMultiplier = 4;
    return weakenTimeMultiplier * utilCalculateHackingTime(server, player) * 1000;
}
 
export function getGrowTime(server: Server, player: Player) {
 
    const growTimeMultiplier = 3.2;
    return growTimeMultiplier * utilCalculateHackingTime(server, player) * 1000;
}
 
export function getHackTime(server: Server, player: Player) {
 
    return utilCalculateHackingTime(server,player) * 1000;
}
 
function utilCalculateIntelligenceBonus(intelligence: number, weight = 1) {
    return 1 + (weight * Math.pow(intelligence, 0.8)) / 600;
}
 
function utilCalculateHackingTime(server: Server, player: Player) {
